...
| `validateAll(mixed $value)` | Run shared and password-specific checks. |
...
$errors = $validator->validateAll('abc');
```

Password checks include uppercase, lowercase, number, special character, and length requirements. Use `validateAll()` to show a complete checklist to a user.